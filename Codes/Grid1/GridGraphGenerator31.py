...
def GridGraphGenerator():
    ...
    cur.execute('select distinct node from public."GridTest"\
    ...
    for node in nodeName:
        ...
        cur.execute('select id ,centerx, centery from public."GridTest"\
        ...
        nodeGridTest = np.zeros([np.unique(data[:,2]).shape[0],np.unique(data[:,1]).shape[0]])
        ...
        for i in range(data.shape[0]):
            nodeGridTest[tupleX[data[i,2]], tupleY[data[i,1]]] = data[i,0]
        name = 'itsmine3'+  str(node[0])+'.csv'     
        np.savetxt(name, nodeGridTest, delimiter=',', fmt='%d')
        #plt.figure(2)
        #plt.plot(data[:,1], data[:,2], 'co')
        np.savetxt('itsmine2.csv', data, delimiter=',', fmt = '%f')
        np.savetxt('itsmine.csv', nodeName, delimiter=',', fmt='%d')
...
#    nodesTransitionCells = pd.read_csv('TransitionCellsAtr.csv', header=None)
...
#    conn.rollback()
#    
#    cur.execute('select distinct edge from public."myDeawingFullCellOneDimension"\
...
#        conn.rollback()
#        
#        cur.execute('select id ,centerx, centery from public."myDeawingFullCellOneDimension"\
...
#        data = np.asarray(data)   # break down each tuple to 3 numpy array component
#        
#        edgeGridTest = np.zeros([np.unique(data[:,2]).shape[0],np.unique(data[:,1]).shape[0]])
#        # according to cell coordinate the number of row and column in matrix are calculated
#        tupleX = {}
#    
#        tupleY = {}
...
#        
#        for i in range(data.shape[0]):
#            edgeGridTest[tupleX[data[i,2]], tupleY[data[i,1]]] = data[i,0]
#        name = 'edge'+  str(edge[0])+'.csv'     
#        np.savetxt(name, edgeGridTest, delimiter=',', fmt='%d')
