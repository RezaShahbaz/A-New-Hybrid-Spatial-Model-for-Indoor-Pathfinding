...
#        conn.rollback()
#        cur.execute('select centerx, centery from public."GridGraph" where id = '+s)
#        temp1 = cur.fetchall()
...
#    plt.plot(obsx, obsy, 'rs', markersize = 3)
    ...
    shape = grid.shape
    conn.rollback()
    cur.execute('select distinct width, height from public."GridGraph" where node = ' + str(gridNum))
    dim = np.asarray(cur.fetchall())
    if len(dim) == 0 :
        cur.execute('select distinct width, height from public."GridGraph" where edge = ' + str(gridNum))
        dim = np.asarray(cur.fetchall())
    def hvalue(point):
        global dim
...
################# just for plot#########################
    ...
    for p in closeList:
        cIds.append(grid[p])
    for p in cIds:
        cur.execute('select centerx, centery from public."GridGraph"\
                    where id = '+str(p))
        data = cur.fetchall()
