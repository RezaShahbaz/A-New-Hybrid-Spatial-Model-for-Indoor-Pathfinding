...
def transitionCellFinder():
    ...
    cur.execute('select id,bleft, bright,\
                btop, bbottom, Node, edge from public."GridGraph"\
...
#    plt.figure()
    for i in range(info.shape[0]):
        poly = sg.box(info[i, 1]-0.005, info[i, 4]-0.005, info[i, 2]+0.005, info[i, 3]+0.005, ccw = True)
...
#    plt.show()
    ...
    for poly in polyList:
        ...
        for poly1 in polyList:
            if poly[1].touches(poly1[1]) or poly[1].overlaps(poly1[1]):
                if str(info[i, 5]) !='nan':
    ...
    conn.close()
    cur.close()
    np.savetxt('F:\Stuff\Current_Term\KNTU\papers\ISI\Codes\GridGraph\TransitionCellsAtr.csv', transitionCellsAtr, delimiter=',', fmt='%d')
