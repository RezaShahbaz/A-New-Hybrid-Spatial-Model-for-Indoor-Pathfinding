...
cur.execute('select id, height, width, center  from public."GridGraph"')
...
for inf in info:
    ...
    query = 'update public."GridGraph" set centerx = %s, centery = %s, bleft =%s,bright = %s,btop = %s,bbottom= %s where id = %s'
    ...
    conn.commit()
cur.execute('select id, centerx, centery, bleft, bright, btop, bbottom from public."GridGraph"')
